...
/**
 * Joey's notes:
 * 	-- spacing/indentation as per other files
 *  -- search_box() and column_default() : adjusted the return to be empty instead of a string, no need to make it a string...
 *  -- several functions : adjusted returns and escape all the things
 *  -- process_bulk_action() : needs internationalization
 *  -- prepare_items() : $per_page at 100000 looks like a disaster waiting to happen to me!
 */
...
class TribeEventsTicketsAttendeesTable extends WP_List_Table {
	...
	function column_order_id( $item ) {
		return "<a class='row-title' href='" . esc_url( get_edit_post_link( $item["order_id"], true ) ) . "'>" . $item["order_id"] . "</a>";
	}
	...
	function extra_tablenav( $which ) {
		...
		if ( $which == "top" ) {
			echo sprintf( "%s: <input type='text' name='filter_attendee' id='filter_attendee' value=''>",
			              __( "Filter by ticket #, order # or security code", "tribe-events-calendar" ) );
		}
	}
