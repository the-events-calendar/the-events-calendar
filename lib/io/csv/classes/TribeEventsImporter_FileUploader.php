...
class TribeEventsImporter_FileUploader {
	...
	private function validate_temporary_file() {
		if ( !file_exists($this->tmp_name) ) {
			throw new RuntimeException( sprintf(__('Temporary file not found. Could not save %s.', 'tribe-events-importer'), $this->name));
		}
	}
	...
	private function permanently_save_file() {
		...
		if ( !$moved ) {
			throw new RuntimeException( sprintf(__('Could not save %s.', 'tribe-events-importer'), $this->name));
		}
	}
