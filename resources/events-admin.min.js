events-admin.js:188: WARNING - Suspicious code. This code lacks side-effects. Is there a bug?
...
f=a('[name="recurrence[custom-week-day][]"]:checked').map(function(){return a(this).val()}).get(),r=a('[name="recurrence[custom-month-number]"]').val(),u=a('[name="recurrence[custom-month-day]"]').val(),t=a('[name="recurrence[custom-year-month][]"]:checked').map(function(){return a(this).val()}).get();a('[name="recurrence[custom-year-filter]"]').val();var z=a('[name="recurrence[custom-year-month-number]"]').val(),C=a('[name="recurrence[custom-year-month-day]"]').val(),v="Monday Tuesday Wednesday Thursday Friday Saturday Sunday".split(" "),
...
f.length||0<f[d]-k&&f[d]-k<h[0])for(var y=0,w=d;y<f.length;y++,w++)h[y]=f[w],w+1==f.length&&0<w&&(w=-1);d=g%f.length-1;"-1"==d&&(d=f.length-1);k+1!=h[f.length-1]&&(B=1,d--,"-1"==d&&(d=f.length-1));Math.ceil(g/f.length-B/f.length)}if("Custom"!=c)"After"==n?b+=" and repeats "+c.toLowerCase()+" for "+g+" "+a("#occurence-count-text").text():"On"==n&&""!=e&&(b+=" and repeats "+c.toLowerCase()+" until "+e);else{b+=" and repeats "+l.toLowerCase();b=1<p?b+(" every "+p+" "+s.toLowerCase()):b+(" every "+s.toLowerCase());
if("Weekly"==l&&f&&0<f.length)for(b+=" on ",d=0;d<f.length;d++)0==d?b+=v[f[d]-1]:d!=f.length-1&&0<d?b+=", "+v[f[d]-1]:(2<f.length&&(b+=","),b+=" and "+v[f[d]-1]);"Monthly"==l&&(b+=" on",isNaN(r)?(b+=" the "+r.toLowerCase(),b="-1"==u?b+" day":b+(" "+v[u-1])):b+=" day "+r);if("Yearly"==l){b+=" in ";for(d=0;d<t.length;d++)0==d?b+=x[t[d]-1]:d!=t.length-1&&0<d?b+=", "+x[t[d]-1]:(2<t.length&&(b+=","),b+=" and "+x[t[d]-1]);a('[name="recurrence[custom-year-filter]"]').is(":checked")&&(b+=" on the ",b="-1"==
z?b+A[0]:b+A[z],b+=" "+v[C-1])}"After"==n?b+=" for "+g+" occurrences":"On"==n&&""!=e&&(b+=" until "+e)}b+=".";"After"==n&&"Custom"!=c&&(b+=" The final occurrence begins",b=m.getTime()===q.getTime()?b+(" and ends on "+m.getFullYear()+"-"+(m.getMonth()+1)+"-"+m.getDate()+"."):b+(" on "+q.getFullYear()+"-"+(q.getMonth()+1)+"-"+q.getDate()+" and ends on "+m.getFullYear()+"-"+(m.getMonth()+1)+"-"+m.getDate()+"."));"None"==c&&a(".recurrence-pattern-description-row").hide();a("#recurrence-pattern-description").text(b)}
...
"On"==a('[name="recurrence[end-type]"] option:selected').val()?a('[name="recurrence[end]"]').val()&&!a('[name="recurrence[end]"]').hasClass("placeholder"):!0;e||(c.preventDefault(),alert(a("#rec-end-error").text()),a("#rec-end-error").show(),r())});a(".recurrence-row, .custom-recurrence-row").on("change",function(c){a(".recurrence-pattern-description-row").is(":visible")||a(".recurrence-pattern-description-row").show();u()});a("#recurrence_end, #EventStartDate, #EventEndDate").datepicker("option",
"onClose",u);a('input[name="post[]"]').click(function(c){c=a(this).val();a(this).is(":checked")?a('input[name="post[]"][value="'+c+'"]').prop("checked",!0):a('input[name="post[]"][value="'+c+'"]').prop("checked",!1)})});
