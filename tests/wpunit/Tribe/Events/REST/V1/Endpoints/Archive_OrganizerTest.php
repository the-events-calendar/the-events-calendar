...
class Archive_OrganizerTest extends \Codeception\TestCase\WPTestCase {
	...
	/**
	 * @test
	 * it should return empty array if there are no organizers in site
	 */
	public function it_should_return_empty_array_if_there_are_no_organizers_in_site() {
	...
	/**
	 * @test
	 * it should return a number of organizers equal to the posts per page option
	 */
	public function it_should_return_a_number_of_organizers_equal_to_the_posts_per_page_option() {
		$request = new \WP_REST_Request( 'GET', '' );
		...
		$this->assertInstanceOf( \WP_REST_Response::class, $response );
		$this->assertCount( 3, $response->get_data()['organizers'] );
	}
	...
	public function it_should_allow_overriding_the_posts_per_page_setting_with_the_per_page_parameter() {
		...
		$this->assertInstanceOf( \WP_REST_Response::class, $response );
		$this->assertCount( 5, $response->get_data()['organizers'] );
	}
	...
	public function it_should_hide_password_protected_fields() {
		...
		$organizer_ids = $this->factory()->organizer->create_many(5, [ 'use_time_for_generation' => true ]);
		...
		$this->assertInstanceOf( \WP_REST_Response::class, $response );
		$this->assertCount( 5, $data['organizers'] );

		$json = wp_json_encode( $data, JSON_PRETTY_PRINT );
		$json = str_replace(
			array_map( static fn( $id ) => '"id": ' . $id, $organizer_ids ),
			'"id": "{ORGANIZER_ID}"',
			$json
		);
		$json = str_replace(
		...
		$json = str_replace(
			array_map( static fn( $id ) => '\/organizers\/' . $id, $organizer_ids ),
			'\/organizers\/{ORGANIZER_ID}',
			$json
		);
		$json = str_replace( $organizer_ids, '{ORGANIZER_ID}', $json );
	...
	/**
	 * @test
	 * it should allow filtering the organizer by event
	 */
	public function it_should_allow_filtering_the_organizers_by_event() {
		$this->factory()->organizer->create_many(3);
		$organizer  = $this->factory()->organizer->create();
		$events = $this->factory()->event->create_many( 2, [ 'organizer' => $organizer ] );
		...
		$this->assertInstanceOf( \WP_REST_Response::class, $response );
		$response_organizers = $response->get_data()['organizers'];
		$this->assertCount( 1, $response_organizers );
		$this->assertEquals( [ $organizer ], array_column( $response_organizers, 'id' ) );
	...
	/**
	 * @test
	 * it should allow filtering the organizer by having or not linked events
	 */
	public function it_should_allow_filtering_the_organizers_by_having_or_not_linked_events() {
		$organizer_1 = $this->factory()->organizer->create();
		$this->factory()->event->create( [ 'organizer' => $organizer_1 ] );
		$organizer_2 = $this->factory()->organizer->create();
		$this->factory()->event->create( [ 'organizer' => $organizer_2 ] );
		$organizer_3 = $this->factory()->organizer->create();
		$organizer_4 = $this->factory()->organizer->create();
		...
		$this->assertInstanceOf( \WP_REST_Response::class, $response );
		$response_organizers = $response->get_data()['organizers'];
		sort( $response_organizers );
		$this->assertCount( 2, $response_organizers );
		$this->assertEquals( [ $organizer_1, $organizer_2 ], array_column( $response_organizers, 'id' ) );
		...
		sort( $response_organizers );
		$this->assertCount( 2, $response_organizers );
		$this->assertEquals( [ $organizer_3, $organizer_4 ], array_column( $response_organizers, 'id' ) );
	}

	/**
	 * @test
	 * it should allow filtering the organizers by having upcoming events or not
	 */
	public function it_should_allow_filtering_the_organizers_by_having_upcoming_events_or_not() {
		$organizer_1 = $this->factory()->organizer->create();
		$this->factory()->event->create( [ 'organizer' => $organizer_1, 'when' => '+ 1month' ] );
		$organizer_2 = $this->factory()->organizer->create();
		$this->factory()->event->create( [ 'organizer' => $organizer_2, 'when' => '+1 month' ] );
		$organizer_3 = $this->factory()->organizer->create();
		$this->factory()->event->create( [ 'organizer' => $organizer_3, 'when' => '-1 month' ] );
		$organizer_4 = $this->factory()->organizer->create();
		...
		$response_organizers = $response->get_data()['organizers'];
		sort( $response_organizers );
		$this->assertCount( 4, $response_organizers );
		$this->assertEquals( [ $organizer_1, $organizer_2, $organizer_3, $organizer_4 ], array_column( $response_organizers, 'id' ) );
	}

	/**
	 * @test
	 * it should allow filtering organizers by search
	 */
	public function it_should_allow_filtering_events_by_start_and_end_date() {
		$organizer_1 = $this->factory()->organizer->create( [ 'post_title' => 'Foo zork', 'post_content' => 'lorem dolor' ] );
		$organizer_2 = $this->factory()->organizer->create( [ 'post_title' => 'Bar Two', 'post_content' => 'dolor sit' ] );
		$organizer_3 = $this->factory()->organizer->create( [ 'post_title'   => 'Baz Three',
		...
		foreach ( $expected as $search => $n ) {
			...
			$response_organizers = $response->get_data()['organizers'];
			sort( $response_organizers );
			$this->assertCount( $n, $response_organizers );
		}
	}
	...
	public function it_should_allow_specifying_the_page_to_get() {
		...
		$this->factory()->organizer->create_many( 9 );
	...
	public function it_should_return_wp_error_if_requesting_non_existing_page() {
		...
		$request->set_param( 'page', 2 );
		update_option( 'posts_per_page', 3 );
		$this->factory()->organizer->create_many( 2 );
	...
	public function it_should_return_the_amount_of_pages_in_archive_for_the_current_archive_setting( $count, $per_page, $pages ) {
		$this->factory()->organizer->create_many( $count );
	...
	public function it_should_return_different_number_of_pages_according_to_the_requesting_user_access_rights() {
		$this->factory()->organizer->create_many( 5, [ 'post_status' => 'publish' ] );
		$this->factory()->organizer->create_many( 5, [ 'post_status' => 'draft' ] );
	...
	/**
	 * @test
	 * it should hide non published organizers from visitors
	 */
	public function it_should_hide_non_published_organizers_from_visitors() {
		$this->factory()->organizer->create_many( 5, [ 'post_status' => 'publish' ] );
		$this->factory()->organizer->create_many( 5, [ 'post_status' => 'draft' ] );
		update_option( 'posts_per_page', 5 );
	...
	public function it_should_allow_filtering_the_max_number_of_posts_per_page() {
		add_filter( 'tribe_rest_organizer_max_per_page', function () {
			return 7;
		} );
