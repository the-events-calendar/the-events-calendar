...
use Tribe__Events__REST__V1__Endpoints__Archive_Tag as Archive;

class Archive_TagTest extends \Codeception\TestCase\WPTestCase {
	...
	/**
	 * It should return existing event tags
	 *
	 * @test
	 */
	public function should_return_existing_event_tags() {
		...
		$this->repository->prepare_terms_data( Argument::type( 'array' ), 'post_tag')->willReturn( [ 'some' => 'prepared_data' ] );
		$sut     = $this->make_instance();
		...
		$this->assertEquals( [ 'some' => 'prepared_data' ], $got->data['tags'] );
	}

	/**
	 * It should return WP_Error ir there are no event tags in db
	 *
	 * @test
	 */
	public function should_return_wp_error_ir_there_are_no_event_tags_in_db() {
