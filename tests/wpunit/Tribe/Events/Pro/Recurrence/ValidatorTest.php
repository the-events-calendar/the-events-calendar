...
use Tribe__Events__Pro__Recurrence__Validator as Validator;
...
class ValidatorTest extends \Codeception\TestCase\WPTestCase {
	 ...
	 */
	public function it_should_return_false_if_custom_recurrence_is_missing_data() {
		foreach ( Custom_Types::data_keys() as $key ) {
			$post_id = $this->get_event();
	...
	public function it_should_return_false_if_monthly_recurrence_is_missing_day_or_number() {
		...
		$valid = $sut->is_valid( $post_id,
			array(
				'type'   => Custom_Types::SLUG,
				'custom' => array(
					'type'  => Custom_Types::MONTHLY_CUSTOM_TYPE,
					'month' => array()
				),
			) );
