...
class Tribe__Events__Updater_Test extends Tribe__Events__WP_UnitTestCase {
	public function test_update_3_dot_8() {
		TribeEvents::instance()->setOption( 'schema-version', 0 );
	...
	public function test_no_update_for_3_dot_7() {
		TribeEvents::instance()->setOption( 'schema-version', 0 );
		remove_action( 'wp_loaded', 'flush_rewrite_rules' );
		$this->assertFalse( has_action( 'wp_loaded', 'flush_rewrite_rules' ) );
		$updater = new Tribe__Events__Updater( '3.7' );
		$updater->do_updates();
		$this->assertFalse( has_action( 'wp_loaded', 'flush_rewrite_rules' ) );
	}
