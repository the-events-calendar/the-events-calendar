...
class Venues_Test extends Venue_Test {
	...
	public function test_read_responses( Closure $fixture ) {
		...
		// Ensure we have all of the published venues in the response.
		$this->assertEquals( $expected_count, count( $response ) );

		// Some venues aren't published, so also assert that we have less that the total in the response.
		$this->assertLessThan( count( $venues ), count( $response ), 'There should be fewer venues in the response.' );
