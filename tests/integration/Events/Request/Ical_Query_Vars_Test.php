...
/**
 * Class Ical_Query_Vars_Test
 *
 * @since TBD
 */
class Ical_Query_Vars_Test extends \Codeception\TestCase\WPTestCase {
    ...
    public function before() {
        parent::before();
        // Clear superglobals to ensure a clean test environment.
        $_GET = [];
        $_POST = [];
        $_REQUEST = [];
    }
    ...
    public static function truthy_values_provider() {
        return [
            // Presence only (?ical) and null values are treated as 1.
            [ '' ],
            [ null ],
            [ '1' ],
            [ 1 ],
    ...
    /**
     * Provides falsey/invalid values for the ical query var that should result in unsetting.
     *
     * @return array
     */
