<?php return '<div  class="tribe-events-header__messages tribe-events-c-messages tribe-common-b2"  >
			<div class="tribe-events-c-messages__message tribe-events-c-messages__message--notice" tabindex="0" role="alert" aria-live="assertive">
			<svg
	 class="tribe-common-c-svgicon tribe-common-c-svgicon--messages-not-found tribe-events-c-messages__message-icon-svg" 	aria-hidden="true"
	viewBox="0 0 21 23"
	xmlns="http://www.w3.org/2000/svg"
>
	<g fill-rule="evenodd">
		<path d="M.5 2.5h20v20H.5z"/>
		<path stroke-linecap="round" d="M7.583 11.583l5.834 5.834m0-5.834l-5.834 5.834" class="tribe-common-c-svgicon__svg-stroke"/>
		<path stroke-linecap="round" d="M4.5.5v4m12-4v4"/>
		<path stroke-linecap="square" d="M.5 7.5h20"/>
	</g>
</svg>
<span class="tribe-common-a11y-visual-hide">
	Notice</span>
							<div
					 data-key="0" >
					There were no results found				</div>
					</div>
			<div class="tribe-events-c-messages__message tribe-events-c-messages__message--error" tabindex="0" role="alert" aria-live="assertive">
			<svg
	 class="tribe-common-c-svgicon tribe-common-c-svgicon--error tribe-events-c-messages__message-icon-svg" 	aria-hidden="true"
	viewBox="0 0 18 18"
	xmlns="http://www.w3.org/2000/svg"
>
	<g fill-rule="evenodd" transform="translate(1 1)">
		<circle cx="8" cy="8" r="7.467" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" class="tribe-common-c-svgicon__svg-stroke"/>
		<circle cx="8" cy="11.733" r="1.067" fill-rule="nonzero" class="tribe-common-c-svgicon__svg-fill"/>
		<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 3.733v4.8" class="tribe-common-c-svgicon__svg-stroke"/>
	</g>
</svg>
<span class="tribe-common-a11y-visual-hide">
	Error</span>
							<div
					 data-key="0" >
					The first rule of fight club is, you do not talk about fight club.				</div>
							<div
					 data-key="1" >
					The second rule of fight club is, <strong>you do not talk about fight club.</strong>				</div>
					</div>
	</div>
';
