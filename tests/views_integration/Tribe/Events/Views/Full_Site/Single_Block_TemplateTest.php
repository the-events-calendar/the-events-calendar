...
class Single_Block_TemplateTest extends WPTestCase {
	...
	public function setUp() {
		...
		/* Our template filters only load on a block theme, so a classic one would test nothing. */
		$this->assertTrue( tec_is_full_site_editor(), 'This suite must run on a block theme.' );
	}
	...
	public function test_a_site_editor_save_updates_the_canonical_post_in_place(): void {
		...
		$this->assertSame( [ self::SLUG ], $this->get_live_slugs_claiming_our_template() );
	...
	private function get_posts_claiming_slug(): array {
		return $this->query_namespace( [ 'post_name__in' => [ self::SLUG ] ] );
	}

	/**
	 * Returns the slugs every non-trashed post in our namespace holds, numbered copies included.
	 *
	 * A save that lands on a new row shows up here as the `{slug}-N` WordPress renamed it to, which
	 * an exact-slug lookup would never see.
	 *
	 * @return string[] The matching slugs, in creation order.
	 */
	private function get_live_slugs_claiming_our_template(): array {
		$slugs = [];

		foreach ( $this->query_namespace() as $post_id ) {
			$slug = get_post( $post_id )->post_name;

			if ( 0 !== strpos( $slug, self::SLUG ) ) {
				continue;
			}
	...
	/**
	 * Reads the non-trashed `wp_template` posts filed under our `wp_theme` term.
	 *
	 * @param array $args Extra query arguments to narrow the lookup.
	 *
	 * @return int[] The matching post IDs, in creation order.
	 */
	private function query_namespace( array $args = [] ): array {
		$query = new WP_Query(
			array_merge(
				[
					'post_type'      => 'wp_template',
