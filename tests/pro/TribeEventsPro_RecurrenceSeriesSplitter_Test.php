...
class TribeEventsPro_RecurrenceSeriesSplitter_Test extends WP_UnitTestCase {
	 ...
	 * @todo
	 */
	public function test_break_while_saving_post() {
		$start_date = date('Y-m-d', strtotime('2014-05-01'));
		$event_args = array(
			'post_type' => TribeEvents::POSTTYPE,
			'post_title' => __CLASS__,
		...
		$child_to_break = $original_children[2]; // 2014-05-22
		$event_args['EventStartDate'] = $event_args['EventEndDate'] = date('Y-m-d', strtotime('2014-05-22'));
		$event_args['recurrence_action'] = TribeEventsRecurrenceMeta::UPDATE_TYPE_SINGLE;

		TribeEventsApi::updateEvent($child_to_break, $event_args);
		$updated_children = get_posts(array(
			'post_type' => TribeEvents::POSTTYPE,
			'post_parent' => $post_id,
			'post_status' => 'publish',
