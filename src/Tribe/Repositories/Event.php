...
class Tribe__Events__Repositories__Event extends Tribe__Repository {
	...
	/**
	 * Based on the query filters, iterate on the events and align any UTC dates
	 * with the _EventStartDate and _EventEndDate values.
	 *
	 * @since TBD
	 *
	 * @return int How many events were updated. 0 implies no events found or out of sync.
	 */
	public function sync_utc_dates(): int {
		$ids     = $this->get_ids();
		...
		$ids = array_map( static function ( $id ) {
			/**
			 * Gives the sync operations a chance to filter the IDs to a valid / correct ID value.
			 *
			 * @since TBD
			 *
			 * @param int $id The id to potentially normalize/resolve to a different value.
			 */
			return apply_filters( 'tec_events_sync_utc_dates_resolve_id', $id );
		}, $ids );
		$ids = array_unique( $ids );

		// Now update each event's UTC dates with what is in the non-UTC start/end dates.
		foreach ( $ids as $id ) {
			$did_update = false;
			foreach ( [ '_EventStartDate', '_EventEndDate' ] as $date_field ) {
				$utc_date_field = $date_field . 'UTC';
				// Because an update may have bypassed local cache and could have updated the db directly.
				wp_cache_delete( $id, 'post_meta' );
				$meta = [
					// e.g. _EventStartDate
					$date_field      => get_post_meta( $id, $date_field, true ),
					// e.g. _EventStartDateUTC
					$utc_date_field  => get_post_meta( $id, $utc_date_field, true ),
					'_EventTimezone' => get_post_meta( $id, '_EventTimezone', true ),
				];
				// Any date value to inspect?
				if ( ! empty( $meta[ $date_field ] ) ) {
					$is_utc_offset = Tribe__Events__Timezones::is_utc_offset( $meta['_EventTimezone'] );
					...
					if ( $is_utc_offset ) {
						...
						$new_utc_date = Tribe__Date_Utils::immutable( $meta[ $date_field ], $meta['_EventTimezone'] )
						                                 ->setTimezone( $utc_tz )
						                                 ->format( Tribe__Date_Utils::DBDATETIMEFORMAT );
					} else {
						$new_utc_date = Tribe__Events__Timezones::to_utc( $meta[ $date_field ], $meta['_EventTimezone'] );
					}

					// If we have a valid UTC date to update, sync it.
					if ( ! empty( $new_utc_date ) && $new_utc_date !== $meta[ $utc_date_field ] ) {
						update_post_meta( $id, $utc_date_field, $new_utc_date );
						$did_update = true;
					}
				}
			}
		...
		// Let them know how many events we updated - 0 implies all events were sync'd already.
		return $updated;
	}
}
