...
class Tribe__Events__Admin__Organizer_Chooser_Meta_Box {
	...
	protected function single_organizer_dropdown( $organizer_id ) {
		...
		<tr class="saved_organizer">
			<td style="width:170px"><?php
				$this->move_handle();
				?><label data-l10n-create-organizer="<?php esc_attr( printf( __( 'Create New %s', 'tribe-events-calendar' ), sanitize_title( $this->tribe->singular_organizer_label ) ) ); ?>"><?php printf( esc_html__( 'Use Saved %s:', 'tribe-events-calendar' ), sanitize_title( $this->tribe->singular_organizer_label ) ); ?></label>
			</td>
			<td><?php
	...
	protected function edit_organizer_link( $organizer_id ) {
		...
		<div class="edit-organizer-link"><a
				...
				target="_blank"><?php printf( esc_attr__( 'Edit %s', 'tribe-events-calendar' ), sanitize_title( $this->tribe->singular_organizer_label ) ); ?></a>
		</div>
		<?php
	}
	...
	protected function render_add_organizer_button() {
		printf( '<tfoot><tr><td colspan="2"><a class="tribe-add-organizer" href="#">%s</a></td></tr></tfoot>', esc_html__( 'Add another organizer', 'tribe-events-calendar' ) );
	}
