	...
	class Tribe__Events__Options_Exception extends Exception {
		...
		public static function displayMessage() {
			...
			if ( isset( $eventsOptions['error'] ) && $eventsOptions['error'] ) : ?>
				<script type="text/javascript">jQuery('#tribe-events-options-error').append('<h2><?php esc_html_e( 'Error', 'tribe-events-calendar' ); ?></h2><p>' + '<?php echo $eventsOptions['error']; ?>' + '</p>').show();</script>
			<?php endif;
		}
