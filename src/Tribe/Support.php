...
if ( ! class_exists( 'Tribe__Events__Support' ) ) {
	...
	class Tribe__Events__Support {
		...
		public function displayHelpTabInfo() {
			...
			$system_text[] = '<p>' . esc_html__( "Sometimes it's hard to tell what's going wrong without knowing more about your system setup. For your convenience, we've put together a little report on what's cooking under the hood.", 'the-events-calendar' ) . '</p>';
			$system_text[] = '<p>' . esc_html__( "If you suspect that the problem you're having is related to another plugin, or we're just plain having trouble reproducing your bug report, please copy and send all of this to our support team.", 'the-events-calendar' ) . '</p>';
		...
		public function getSupportStats() {
			...
			$systeminfo = array(
				...
				'WordPress timezone' => get_option( 'timezone_string', esc_html__( 'Unknown or not set', 'the-events-calendar' ) ),
				'server timezone'    => date_default_timezone_get(),
			);

			if ( $this->rewrite_rules_purged ) {
				$systeminfo['rewrite rules purged'] = esc_html__( 'Rewrite rules were purged on load of this help page. Chances are there is a rewrite rule flush occurring in a plugin or theme!', 'the-events-calendar' );
			}
