	...
	class Tribe__Events__Template__Day extends Tribe__Events__Template_Factory {
			...
			if ( $wp_query->have_posts() ) {
				...
				foreach ( $unsorted_posts as &$post ) {
					if ( tribe_event_is_all_day( $post->ID ) ) {
						$post->timeslot = esc_html__( 'All Day', 'tribe-events-calendar' );
					} else {
						if ( strtotime( tribe_get_start_date( $post->ID, true, Tribe__Events__Date_Utils::DBDATETIMEFORMAT ) ) < strtotime( $wp_query->get( 'start_date' ) ) ) {
							$post->timeslot = esc_html__( 'Ongoing', 'tribe-events-calendar' );
						} else {
				...
				foreach ( $unsorted_posts as $i => $post ) {
					if ( $post->timeslot == esc_html__( 'All Day', 'tribe-events-calendar' ) ) {
						$all_day[ $i ] = $post;
					} else {
						if ( $post->timeslot == esc_html__( 'Ongoing', 'tribe-events-calendar' ) ) {
							$ongoing[ $i ] = $post;
						} else {
							$hourly[ $i ] = $post;
						}
		...
		protected function nothing_found_notice() {
			...
			if ( empty( $search_term ) && empty( $geographic_term ) && ! empty( $tax_term ) ) {
				Tribe__Events__Main::setNotice( 'events-not-found', sprintf( esc_html__( 'No matching %1$s listed under %2$s scheduled for %3$s. Please try another day.', 'tribe-events-calendar' ), strtolower( $events_label_plural ), $tax_term, '<strong>' . date_i18n( tribe_get_date_format( true ), strtotime( get_query_var( 'eventDate' ) ) ) . '</strong>' ) );
			} elseif ( empty( $search_term ) && empty( $geographic_term ) ) {
				Tribe__Events__Main::setNotice( 'events-not-found', sprintf( esc_html__( 'No %1$s scheduled for %2$s. Please try another day.', 'tribe-events-calendar' ), strtolower( $events_label_plural ), '<strong>' . date_i18n( tribe_get_date_format( true ), strtotime( get_query_var( 'eventDate' ) ) ) . '</strong>' ) );
			} else {
