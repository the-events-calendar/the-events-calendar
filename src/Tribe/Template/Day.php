	...
	class Tribe__Events__Template__Day extends Tribe__Events__Template_Factory {
		...
		protected function nothing_found_notice() {
			...
			if ( empty( $search_term ) && empty( $geographic_term ) && ! empty( $tax_term ) ) {
				Tribe__Notices::set_notice( 'events-not-found', sprintf( __( 'No matching %1$s listed under %2$s scheduled for <strong>%3$s</strong>. Please try another day.', 'the-events-calendar' ), strtolower( $events_label_plural ), $tax_term, date_i18n( tribe_get_date_format( true ), strtotime( get_query_var( 'eventDate' ) ) ) ) );
			} elseif ( empty( $search_term ) && empty( $geographic_term ) ) {
				Tribe__Notices::set_notice( 'events-not-found', sprintf( __( 'No %1$s scheduled for <strong>%2$s</strong>. Please try another day.', 'the-events-calendar' ), strtolower( $events_label_plural ), date_i18n( tribe_get_date_format( true ), strtotime( get_query_var( 'eventDate' ) ) ) ) );
			} else {
