...
class Tribe__Events__Pro__Recurrence__Validator {
	...
	protected function is_custom() {
		return isset( $this->recurrence_meta['type'] ) && 'Custom' === $this->recurrence_meta['type'];
	}
	...
	protected function ensure_all_data() {
		if ( ! ( isset( $this->recurrence_meta['custom']['start-time'] ) || isset( $this->recurrence_meta['custom']['day'] ) || isset( $this->recurrence_meta['custom']['week'] ) || isset( $this->recurrence_meta['custom']['month'] ) || isset( $this->recurrence_meta['custom']['year'] ) ) ) {

			throw new RuntimeException( __( 'Custom recurrences must have all data present.', 'tribe-events-calendar-pro' ) );
		}
	}

	private function ensure_monthly_day() {
		if ( 'Monthly' === $this->recurrence_meta['custom']['type'] && ( empty( $this->recurrence_meta['custom']['month']['day'] ) || empty( $this->recurrence_meta['custom']['month']['number'] ) || '-' === $this->recurrence_meta['custom']['month']['day'] || '' === $this->recurrence_meta['custom']['month']['number'] ) ) {
	...
	private function ensure_yearly_day() {
		if ( 'Yearly' === $this->recurrence_meta['custom']['type'] && ( empty( $this->recurrence_meta['custom']['year']['month-day'] ) || '-' === $this->recurrence_meta['custom']['year']['month-day'] ) ) {
