...
class Tribe__Events__Tickets__Tickets_Pro {
	...
	public function attendees_row_action( $actions ) {
		...
		if ( $post->post_type == Tribe__Events__Main::POSTTYPE ) {
			...
			$actions['tickets_attendees'] = sprintf( '<a title="%s" href="%s">%s</a>', esc_html__( 'See who purchased tickets to this event', 'the-events-calendar' ), esc_url( $url ), esc_html__( 'Attendees', 'the-events-calendar' ) );
		}
	...
	public function attendees_page_load_css_js( $hook ) {
		...
		$mail_data = array(
			...
			'required'        => esc_html__( 'You need to select a user or type a valid email address', 'the-events-calendar' ),
			'sending'         => esc_html__( 'Sending...', 'the-events-calendar' ),
	...
	public function attendees_page_load_pointers( $hook ) {
		...
		if ( version_compare( get_bloginfo( 'version' ), '3.3', '>' ) && ! in_array( 'attendees_filters', $dismissed ) ) {
			$pointer = array(
				...
				'options'    => array(
					'content' => sprintf( '<h2> %s </h2> <p> %s </p>', esc_html__( 'Columns', 'the-events-calendar' ), esc_html__( 'You can use Screen Options to select which columns you want to see. The selection works in the table below, in the email, for print and for the CSV export.', 'the-events-calendar' ) ),
			...
			foreach ( $item as $key => $data ) {
				if ( in_array( $key, $export_columns ) ) {
					if ( $key == 'check_in' && $data == 1 ) {
						$data = esc_html__( 'Yes', 'the-events-calendar' );
					}
					$row[ $key ] = $data;
				}
			}
	...
	public function ajax_handler_attendee_mail_list() {
		...
		if ( ! wp_mail( $email, sprintf( esc_html__( 'Attendee List for: %s', 'the-events-calendar' ), $event->post_title ), $content ) ) {
