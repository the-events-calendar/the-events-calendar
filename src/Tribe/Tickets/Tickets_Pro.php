	...
	public function attendees_page_load_pointers( $hook ) {
		...
		if ( version_compare( get_bloginfo( 'version' ), '3.3', '>' ) && ! in_array( 'attendees_filters', $dismissed ) ) {
			$pointer = array(
				...
				'options'    => array(
					'content' => sprintf( '<h2> %s </h2> <p> %s </p>', esc_html__( 'Columns', 'tribe-events-calendar' ), esc_html__( 'You can use Screen Options to select which columns you want to see. The selection works in the table below, in the email, for print and for the CSV export.', 'tribe-events-calendar' ) ),
