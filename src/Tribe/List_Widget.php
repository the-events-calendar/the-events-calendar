...
class Tribe__Events__List_Widget extends WP_Widget {
	...
	public function __construct( $id_base = '', $name = '', $widget_options = array(), $control_options = array() ) {
		$widget_options = array_merge(
			array(
				...
				'description' => esc_html__( 'A widget that displays upcoming events.', 'tribe-events-calendar' ),
			),
			$widget_options
		);
		...
		$name    = empty( $name ) ? esc_html__( 'Events List', 'tribe-events-calendar' ) : $name;
	...
	public function form( $instance ) {
		...
		$defaults  = array(
			'title'              => esc_html__( 'Upcoming Events', 'tribe-events-calendar' ),
			'limit'              => '5',
