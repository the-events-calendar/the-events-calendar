	...
	class Tribe__Events__Validate {
		...
		public function __construct( $field_id, $field, $value, $additional_args = array() ) {
			...
			if ( ! is_array( $this->field ) || ( ! isset( $this->field['validation_type'] ) && ! isset( $this->field['validation_callback'] ) ) ) {
				...
				$this->result->error = esc_html__( 'Invalid or incomplete field passed', 'tribe-events-calendar' );
				$this->result->error .= ( isset( $this->field['id'] ) ) ? ' (' . esc_html__( 'Field ID:', 'tribe-events-calendar' ) . ' ' . $this->field['id'] . ' )' : '';
			...
			if ( isset( $this->field['validation_type'] ) ) {
				if ( method_exists( $this, $this->field['validation_type'] ) ) {
					...
					$this->result->valid = false;
					$this->result->error = esc_html__( 'Non-existant field validation function passed', 'tribe-events-calendar' );
					$this->result->error .= ( isset( $this->field['id'] ) ) ? ' (' . esc_html__( 'Field ID:', 'tribe-events-calendar' ) . ' ' . $this->field['id'] . ' ' . _x( 'with function name:', 'non-existant function name passed for field validation', 'tribe-events-calendar' ) . ' ' . $this->field['validation_type'] . ' )' : '';
				}
			} else {
				// no validation type set, validation fails
				$this->result->valid = false;
				$this->result->error = esc_hmtl__( 'Invalid or incomplete field passed', 'tribe-events-calendar' );
				$this->result->error .= ( isset( $this->field['id'] ) ) ? ' (' . esc_html__( 'Field ID:', 'tribe-events-calendar' ) . ' ' . $this->field['id'] . ' )' : '';
			}
		...
		public function alpha_numeric() {
			if ( preg_match( '/^[a-zA-Z0-9]+$/', $this->value ) ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( '%s must contain numbers and letters only', 'tribe-events-calendar' ), $this->label );
			}
		}
		...
		public function alpha_numeric_multi_line_with_dots_and_dashes() {
			if ( preg_match( '/^[a-zA-Z0-9\s.-]+$/', $this->value ) ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( '%s must contain numbers, letters and dots only', 'tribe-events-calendar' ), $this->label );
			}
		}
		...
		public function positive_int() {
			if ( preg_match( '/^[0-9]+$/', $this->value ) && $this->value > 0 ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( '%s must be a positive number.', 'tribe-events-calendar' ), $this->label );
			}
		}
		...
		public function slug() {
			if ( preg_match( '/^[a-zA-Z0-9-_]+$/', $this->value ) ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( '%s must be a valid slug (numbers, letters, dashes, and underscores).', 'tribe-events-calendar' ), $this->label );
			}
		}
		...
		public function url() {
			...
			if ( esc_url_raw( $this->value ) == $this->value ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( '%s must be a valid absolute URL.', 'tribe-events-calendar' ), $this->label );
			}
		}
		...
		public function options() {
			if ( array_key_exists( $this->value, $this->field['options'] ) ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( "%s must have a value that's part of its options.", 'tribe-events-calendar' ), $this->label );
			}
		}
		...
		public function cannot_be_the_same_as() {
			if ( ! isset( $this->additional_args['compare'] ) ) {
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( 'Comparison validation failed because no comparison value was provided, for field %s', 'tribe-events-calendar' ), $this->field['id'] );
			} else {
				if ( $this->value != $this->additional_args['compare'] ) {
					...
					if ( isset( $this->additional_args['compare_name'] ) ) {
						$this->result->error = sprintf( esc_html__( '%1$s cannot be the same as %2$s.', 'tribe-events-calendar' ), $this->label, $this->additional_args['compare_name'] );
					} else {
						$this->result->error = sprintf( esc_html__( '%s cannot be a duplicate', 'tribe-events-calendar' ), $this->label );
					}
				}
			}
		}
		...
		public function number_or_percent() {
			if ( preg_match( '/^[0-9]+%{0,1}$/', $this->value ) ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( '%s must be a number or percentage.', 'tribe-events-calendar' ), $this->label );
			}
		}
		...
		public function google_maps_zoom() {
			if ( preg_match( '/^([0-9]|[0-1][0-9]|2[0-1])$/', $this->value ) ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( '%s must be a number between 0 and 21.', 'tribe-events-calendar' ), $this->label );
			}
		}
		...
		public function address() {
			...
			if ( preg_match( "/^[0-9\S '-]+$/", $this->value ) ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( '%s must consist of letters, numbers, dashes, apostrophes, and spaces only.', 'tribe-events-calendar' ), $this->label );
			}
		}
		...
		public function city_or_province() {
			...
			if ( preg_match( "/^[\D '\-]+$/", $this->value ) ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( '%s must consist of letters, spaces, apostrophes, and dashes.', 'tribe-events-calendar' ), $this->label );
			}
		}
		...
		public function zip() {
			if ( preg_match( '/^[0-9]{5}$/', $this->value ) ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( '%s must consist of 5 numbers.', 'tribe-events-calendar' ), $this->label );
			}
		}
		...
		public function phone() {
			if ( preg_match( '/^[0-9\(\)\+ -]+$/', $this->value ) ) {
				...
				$this->result->valid = false;
				$this->result->error = sprintf( esc_html__( '%s must be a phone number.', 'tribe-events-calendar' ), $this->label );
			}
		}
