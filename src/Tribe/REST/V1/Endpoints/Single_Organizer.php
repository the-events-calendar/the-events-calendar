...
class Tribe__Events__REST__V1__Endpoints__Single_Organizer
	...
	public function can_delete( ?WP_REST_Request $request = null ) {
		...
		$post = get_post( $id );
		if ( ! $post || $post->post_type !== Tribe__Events__Main::ORGANIZER_POST_TYPE ) {
			return false;
		}

		// Check if user can delete others' posts, or if they own this post.
		if ( current_user_can( get_post_type_object( Tribe__Events__Main::ORGANIZER_POST_TYPE )->cap->delete_others_posts ) ) {
			return true;
		}
	...
	public function can_edit( ?WP_REST_Request $request = null ) {
		...
		if ( current_user_can( get_post_type_object( Tribe__Events__Main::ORGANIZER_POST_TYPE )->cap->edit_others_posts ) ) {
			return true;
		}
