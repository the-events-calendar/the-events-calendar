...
class Tribe__Events__Importer__File_Uploader {
	...
	private function validate_temporary_file() {
		if ( ! file_exists( $this->tmp_name ) ) {
			throw new RuntimeException( sprintf( esc_html__( 'Temporary file not found. Could not save %s.', 'tribe-events-calendar' ), $this->name ) );
		}
	}
	...
	private function permanently_save_file() {
		...
		if ( ! $moved ) {
			throw new RuntimeException( sprintf( esc_html__( 'Could not save %s.', 'tribe-events-calendar' ), $this->name ) );
		}
	}
