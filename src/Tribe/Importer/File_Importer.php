...
abstract class Tribe__Events__Importer__File_Importer {
	...
	public static function get_importer( $type, Tribe__Events__Importer__File_Reader $file_reader ) {
		switch ( $type ) {
			...
			default:
				throw new InvalidArgumentException( sprintf( esc_html__( 'No importer defined for %s', 'tribe-events-calendar' ), $type ) );
		}
	}
	...
	protected function import_next_row() {
		...
		if ( ! $this->is_valid_record( $record ) ) {
			$this->log[ $row ] = sprintf( esc_html__( 'Missing required fields in row %d.', 'tribe-events-calendar', $row ) );
			$this->skipped[] = $row;
		...
		try {
			...
			$this->log[ $row ] = sprintf( esc_html__( 'Failed to import record in row %d.', 'tribe-events-calendar' ), $row );
			$this->skipped[] = $row;
		}
	}
	...
	protected function update_or_create_post( array $record ) {
		if ( $id = $this->match_existing_post( $record ) ) {
			...
			$this->updated ++;
			$this->log[ $this->reader->get_last_line_number_read() + 1 ] = sprintf( esc_html__( '%s (post ID %d) updated.', 'tribe-events-calendar' ), get_the_title( $id ), $id );
		} else {
			$id = $this->create_post( $record );
			$this->created ++;
			$this->log[ $this->reader->get_last_line_number_read() + 1 ] = sprintf( esc_html__( '%s (post ID %d) created.', 'tribe-events-calendar' ), get_the_title( $id ), $id );
		}
	}
