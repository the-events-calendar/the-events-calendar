...
class Tribe__Events__Importer__Admin_Page {
	...
	public function register_admin_page() {
		add_submenu_page(
			...
			esc_html__( 'Import', 'tribe-events-calendar' ),
			esc_html__( 'Import', 'tribe-events-calendar' ),
			'import',
			'events-importer',
		...
		switch ( $this->state ) {
			case 'map':
				try {
					...
					$this->errors[] = esc_html__( 'The file went away. Please try again.', 'tribe-events-calendar' );
					$this->state = '';
	...
	public function get_available_tabs() {
		$tabs = array(
			esc_html__( 'General', 'tribe-events-calendar' ) => 'general',
			esc_html__( 'Import: CSV', 'tribe-events-calendar' ) => 'csv-importer',
		);
	...
	private function handle_file_submission() {
		...
		if ( empty( $_POST['import_type'] ) || empty( $_FILES['import_file']['name'] ) ) {
			$this->errors[] = esc_html__( 'We were unable to process your request. Please try again.', 'tribe-events-calendar' );
			$this->state = '';
			return;
		}
	...
	private function handle_column_mapping() {
		...
		if ( ! empty( $missing ) ) {
			...
			$message = '<p>' . esc_html__( 'The following fields are required for a successful import:', 'tribe-events-calendar' ) . '</p>';
