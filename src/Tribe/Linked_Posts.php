...
class Tribe__Events__Linked_Posts {
	...
	public function saved_linked_post_dropdown( $post_type, $current = null ) {
		...
		$placeholder = sprintf( esc_attr__( 'Search a %s', 'the-events-calendar' ), $singular_name );
		if ( $user_can_create ) {
			$placeholder = sprintf( esc_attr__( 'Search or Create a new %s', 'the-events-calendar' ), $singular_name );
		}

		$search_placeholder = sprintf( esc_attr__( 'Select a %s', 'the-events-calendar' ), $singular_name );
		if ( $user_can_create ) {
			$search_placeholder = sprintf( esc_attr__( 'Create or Select a %s', 'the-events-calendar' ), $singular_name );
		}
