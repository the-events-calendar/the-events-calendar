...
if ( class_exists( 'Tribe__Events__Main' ) ) {
	...
	function tribe_get_events_title( $depth = true ) {
		...
		$title = sprintf( esc_html__( 'Upcoming %s', 'tribe-events-calendar' ), $events_label_plural );
		...
		if ( isset( $_REQUEST['tribe-bar-date'] ) && $wp_query->have_posts() ) {
			...
			$title = sprintf( esc_html__( '%1$s for %2$s - %3$s', 'tribe-events-calendar' ), $events_label_plural, $first_event_date, $last_event_date );
		} elseif ( tribe_is_past() ) {
			$title = sprintf( esc_html__( 'Past %s', 'tribe-events-calendar' ), $events_label_plural );
		}

		if ( tribe_is_month() ) {
			$title = sprintf(
				esc_html__( '%1$s for %2$s', 'tribe-events-calendar' ),
				$events_label_plural,
