...
const { toMoment, toDate, toDateNoYear, toTime, isSameYear } = momentUtil;
FORMATS.date =
 ...
 * @return {ReactElement} A React Dom Element null if none.
 */
...
const renderStartDate = ( { start, end } ) => {
	...
	return <span className="tribe-editor__subtitle__headline-date">{ startDate }</span>;
};
...
const renderEndDate = ( { start, end, multiDay } ) => {
	...
	return <span className="tribe-editor__subtitle__headline-date">{ endDate }</span>;
};
...
const renderTimezone = ( props ) => {
	...
	return (
		showTimeZone && (
			<span key="time-zone" className="tribe-editor__time-zone">
				<TimeZone value={ timeZoneLabel } placeholder={ timeZoneLabel } onChange={ setTimeZoneLabel } />
			</span>
		)
	);
};

const renderExtras = ( props ) => <Fragment>{ renderTimezone( props ) }</Fragment>;

const renderContentHook = ( props ) => wpHooks.applyFilters( 'blocks.eventDatetime.contentHook', null, props );
