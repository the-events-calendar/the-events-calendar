...
import { defaultStartMoment, defaultEndMoment } from '@moderntribe/events/data/blocks/datetime/reducer';
...
const mapDispatchToProps = ( dispatch, ownProps ) => {
	...
	return {
		onSelect: ( { from, to } ) => dispatch( dateTimeActions.setDateRange( { from, to }, meta ) ),
		...
		onStartTimePickerChange: ( e ) => dispatch( dateTimeActions.setStartTimeInput( e.target.value ) ),
		...
		onEndTimePickerChange: ( e ) => dispatch( dateTimeActions.setEndTimeInput( e.target.value ) ),
		onEndTimePickerClick: ( value, onClose ) => {
			dispatch( dateTimeActions.setEndTime( value, meta ) );
			onClose();
		},
		onMultiDayToggleChange: ( isMultiDay ) => dispatch( dateTimeActions.setMultiDay( isMultiDay, meta ) ),
	};
};

export default compose( withStore(), connect( mapStateToProps, mapDispatchToProps ) )( EventDateTimeDashboard );
