...
const LockedDates = ( { summary } ) => {
	...
	const initialPast = dates.filter( ( chip ) => chip.status === 'past' );
	const [ showPast, setShowPast ] = useState( initialPast.length <= 6 || initialPast.length === dates.length );
	const [ older, setOlder ] = useState( [] );
	const [ next, setNext ] = useState( summary.next ?? null );
	...
	const loadOlder = async () => {
		...
		setLoading( true );
		setMessage( __( 'Loading older dates…', 'the-events-calendar' ) );
		try {
			const url = new URL( summary.url );
			url.searchParams.set( 'offset', next );
			url.searchParams.set( 'as_of', summary.asOf );
			const response = await fetch( url, {
				headers: { 'X-WP-Nonce': summary.nonce },
			...
			setOlder( ( current ) => [
				...current,
				...page.dates.map( ( chip ) => ( { ...chip, editLink: chip.edit_link } ) ),
			] );
			setNext( page.next );
			setMessage(
				page.next === null
					? __( 'All past dates are displayed.', 'the-events-calendar' )
					: __( 'Older dates loaded.', 'the-events-calendar' )
			);
		} catch ( error ) {
			setMessage( __( 'Could not load older dates. Please try again.', 'the-events-calendar' ) );
		} finally {
			setLoading( false );
		}
	};
		...
		<Fragment>
			<p className="tribe-editor__event-dates__summary">
				{ sprintf(
					...
					_n(
						'%d date is scheduled.',
						'%d dates are scheduled.',
						summary.count ?? dates.length,
						'the-events-calendar'
					),
					summary.count ?? dates.length
				) }
			...
			{ past.length > 0 && (
				<Fragment>
					{ upcoming.length > 0 && past.length > 6 && (
						<Button
							variant="secondary"
							size="small"
							className="tribe-editor__event-dates__toggle"
							aria-expanded={ showPast }
							aria-label={ sprintf( toggleLabel, past.length ) }
						...
						>
							...
							<span aria-hidden="true">{ past.length }</span>
						</Button>
					) }
					{ showPast && (
						<ul
					...
					{ next !== null && (
						<Button variant="secondary" size="small" disabled={ loading } onClick={ loadOlder }>
							{ __( 'Load older dates', 'the-events-calendar' ) }
						</Button>
					) }
			...
			<div className={ classes.join( ' ' ) }>
				...
				>
					...
					{ isOccurrence && config.parentEditLink && (
						<p>
							<a href={ config.parentEditLink }>{ __( 'Edit event details.', 'the-events-calendar' ) }</a>
						</p>
					) }
				</Notice>
