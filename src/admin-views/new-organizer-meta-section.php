...
<?php if ( current_user_can( $organizer_pto->cap->create_posts ) ) { ?>
	...
	<tr class="organizer">
		<td>
			<?php
			esc_html_e( 'Email:', 'tribe-events-calendar' );
			if ( apply_filters( 'tribe_show_organizer_email_obfuscation_alert', true ) ) {
				?>
				<small>
					<?php printf( esc_html__( 'You may want to consider %sobfuscating%s any e-mail address published on your site to best avoid it getting harvested by spammers.', 'tribe-events-calendar' ), '<a href="http://wordpress.org/plugins/tags/obfuscate">', '</a>' ); ?>
				</small>
				<?php
			}
			?>
