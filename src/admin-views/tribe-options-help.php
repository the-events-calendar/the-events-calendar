...
if ( ! is_wp_error( $tec_info ) ) {
	...
	$up_to_date      = ( isset( $tec_info['version'] ) && version_compare( Tribe__Events__Main::VERSION, $tec_info['version'], '<' ) ) ? esc_html__( 'You need to upgrade!', 'tribe-events-calendar' ) : esc_html__( 'You are up to date!', 'tribe-events-calendar' );
}
...
$premium_add_ons[] = array(
	'title' => esc_html__( 'The Events Calendar PRO', 'tribe-events-calendar' ),
	/**
...
$premium_add_ons[] = array(
	'title' => esc_html__( 'The Events Calendar: Eventbrite Tickets', 'tribe-events-calendar' ),
	/**
...
$premium_add_ons[] = array(
	'title' => esc_html__( 'The Events Calendar: Community Events', 'tribe-events-calendar' ),
	/**
...
$premium_add_ons[] = array(
	'title' => esc_html__( 'The Events Calendar: Facebook Events', 'tribe-events-calendar' ),
	/**
...
$premium_add_ons[] = array(
	'title' => esc_html__( 'The Events Calendar: WooCommerce Tickets', 'tribe-events-calendar' ),
	/**
...
$premium_add_ons[] = array(
	'title' => esc_html__( 'The Events Calendar: EDD Tickets', 'tribe-events-calendar' ),
	/**
...
$premium_add_ons[] = array(
	'title' => esc_html__( 'The Events Calendar: WPEC Tickets', 'tribe-events-calendar' ),
	/**
...
$premium_add_ons[] = array(
	'title' => esc_html__( 'The Events Calendar: Shopp Tickets', 'tribe-events-calendar' ),
	/**
...
$premium_add_ons[] = array(
	'title' => esc_html__( 'The Events Calendar: Filter Bar', 'tribe-events-calendar' ),
	/**
...
$getting_started_text = esc_html__( "If you're looking for help with The Events Calendar, you've come to the right place. We are committed to helping make your calendar be spectacular... and hope the resources provided below will help get you there.", 'tribe-events-calendar' );
...
$intro_text[] = '<p>' . esc_html__( "If this is your first time using The Events Calendar, you're in for a treat and are already well on your way to creating a first event. Here are some basics we've found helpful for users jumping into it for the first time:", 'tribe-events-calendar' ) . '</p>';
$intro_text[] = '<ul>';
$intro_text[] = '<li>';
$intro_text[] = sprintf( esc_html__( '%sOur New User Primer%s was designed for folks in your exact position. Featuring both step-by-step videos and written walkthroughs that feature accompanying screenshots, the primer aims to take you from zero to hero in no time.', 'tribe-events-calendar' ), '<a href="' . esc_url( Tribe__Events__Main::$tecUrl . 'knowledgebase/new-user-primer-the-events-calendar-and-events-calendar-pro/' . $ga_query_string ) . '" target="blank">', '</a>' );
$intro_text[] = '</li><li>';
...
$intro_text[] = '</li></ul><p>';
$intro_text[] = esc_html__( "Otherwise, if you're feeling adventurous, you can get started by heading to the Events menu and adding your first event.", 'tribe-events-calendar' );
$intro_text[] = '</p>';
$intro_text   = implode( $intro_text );

$support_text[] = '<p>' . sprintf( esc_html__( "We've redone our support page from the ground up in an effort to better help our users. Head over to our %sSupport Page%s and you'll find lots of great resources, including:", 'tribe-events-calendar' ), '<a href="' . esc_url( Tribe__Events__Main::$tecUrl . 'support/' . $ga_query_string ) . '" target="blank">', '</a>' ) . '</p>';
$support_text[] = '<ul><li>';
$support_text[] = sprintf( esc_html__( '%sTemplate tags, functions, and hooks & filters%s for The Events Calendar &amp; Events Calendar PRO', 'tribe-events-calendar' ), '<a href="http://m.tri.be/fk" target="blank">', '</a>' );
$support_text[] = '</li><li>';
$support_text[] = sprintf( esc_html__( '%sFrequently Asked Questions%s ranging from the most basic setup questions to advanced themer tweaks', 'tribe-events-calendar' ), '<a href="' . esc_url( Tribe__Events__Main::$tecUrl . 'knowledgebase/' . $ga_query_string ) . '" target="blank">', '</a>' );

$support_text[] = '</li><li>';
$support_text[] = sprintf( esc_html__( '%sTutorials%s written by both members of our team and users from the community, covering custom queries, integration with third-party themes and plugins, etc.', 'tribe-events-calendar' ), '<a href="' . esc_url( Tribe__Events__Main::$tribeUrl . 'the-events-calendar-for-wordpress-tutorials/' . $ga_query_string ) . '" target="blank">', '</a>' );
$support_text[] = '</li><li>';
$support_text[] = esc_html__( "Release notes for painting an overall picture of the plugin's lifecycle and when features/bug fixes were introduced.", 'tribe-events-calendar' );
$support_text[] = '</li><li>';
$support_text[] = sprintf( esc_html__( "%sAdd-on documentation%s for all of Modern Tribe's official extensions for The Events Calendar (including WooTickets, Community Events, Eventbrite Tickets, Facebook Events, etc)", 'tribe-events-calendar' ), '<a href="http://m.tri.be/fk" target="blank">', '</a>' );
$support_text[] = '</li></ul>';
...
$forum_text[] = '<p>' . sprintf( esc_html__( 'Written documentation can only take things so far...sometimes, you need help from a real person. This is where our %ssupport forums%s come into play.', 'tribe-events-calendar' ), '<a href="http://wordpress.org/support/plugin/the-events-calendar" target="blank">', '</a>' ) . '</p>';
...
$more_text  = esc_html__( 'More...', 'tribe-events-calendar' );
