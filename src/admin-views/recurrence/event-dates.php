		...
		<td>
			...
			<?php if ( ! $is_occurrence && ! empty( $chips['count'] ) ) : ?>
				...
				<?php if ( count( $chips['past'] ) ) : ?>
					...
					$tec_dates_collapsed  = count( $chips['upcoming'] ) > 0 && count( $chips['past'] ) > 6;
					...
					?>
					<?php if ( $tec_dates_collapsed ) : ?>
					<button type="button" class="button tec-events-recurrence-dates__toggle" aria-expanded="false"
						aria-controls="<?php echo esc_attr( $tec_dates_past_list_id ); ?>"
						aria-label="<?php echo esc_attr( $tec_dates_show_label ); ?>"
						...
						<span aria-hidden="true"><?php echo esc_html( count( $chips['past'] ) ); ?></span>
					</button>
					<?php endif; ?>
					<ul
						...
						<?php echo $tec_dates_collapsed ? 'hidden' : ''; ?>
					>
					...
					<?php if ( null !== ( $chips['next'] ?? null ) ) : ?>
						<button type="button" class="button tec-events-recurrence-dates__load" data-past-dates="
						<?php
						echo esc_attr(
							wp_json_encode(
								[
						...
						?>
																												"><?php esc_html_e( 'Load older dates', 'the-events-calendar' ); ?></button>
						<p class="tec-events-recurrence-dates__load-status description" role="status" aria-live="polite"></p>
					<?php endif; ?>
					<script>
						( function () {
							...
							toggle.addEventListener( 'click', function () {
								...
								toggle.setAttribute( 'aria-label', expanded ? toggle.getAttribute( 'data-show-label' ) : toggle.getAttribute( 'data-hide-label' ) );
