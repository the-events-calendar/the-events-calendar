...
( function ( $, _, obj ) {
	...
	obj.request = async function ( data, $container, overwriteSettings = {} ) {
		...
		// Make the request.
		obj.currentAjaxRequest = await $.ajax( $.extend( settings, overwriteSettings ) );
	...
	obj.shirinkUrlComponents = ( $container, components ) => {
		const {
			home_url,
			rest_url: _rest_url,
			rest_nonce: _rest_nonce,
			rest_method: _rest_method,
	 ...
	 * @since 4.9.2
	 *
	 * @param {Object} data       Data sent from the REST API.
