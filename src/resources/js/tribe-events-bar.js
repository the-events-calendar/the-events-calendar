...
(function( factory ) {
	...
	function Selectability( element ) {
		...
		element.attr( 'tabindex', - 1 ).addClass( 'selectability-offscreen' );
	}

	Selectability.prototype.buildElements = function() {
		this.textbox = $( '<div></div>' ).attr( {
			role: 'textbox',
			tabindex: - 1,
			'aria-readonly': 'true'
		} );

		this.listbox = $( '<div></div>' ).attr( {
			role: 'listbox',
			tabindex: - 1,
			'aria-multiselectable': 'false'
		} );

		this.combobox = $( '<div></div>' ).addClass( 'selectability' ).attr( {
			role: 'combobox application',
			tabindex: 0,
			'aria-expanded': 'false'
		} );

		this.combobox.append( this.textbox ).append( this.listbox );
	...
	Selectability.prototype.stealLabel = function() {
		var id = this.element.attr( 'id' ),
			...
			view = this.element.attr( 'data-view' );
	...
	Selectability.prototype.populateText = function( event ) {
		...
		if ( selected.length ) {
			this.textbox.text(
				selected.attr( 'label' ) || selected.text()
			).prepend( '<span class="tribe-icon ' + selected.text().toLowerCase()  + '"></span>' )
		}
	}
			...
			$.each( elements, function( i, element ) {
				...
				if ( element.is( 'option' ) ) {
					inner.text(
						element.attr( 'label' ) ||
						element.text()
					).prepend( '<span class="tribe-icon ' + element.text().toLowerCase()  + '"></span>' )
					...
					if ( children.children().length ) {
						var label = $( '<div></div>' ).text( element.attr( 'label' ) ).attr( {
							role: 'heading',
							id: idgen()
						} );
						...
						} ).append( children.prepend( label ).children() ).appendTo( node );
					}
				}
			} );
	...
	Selectability.prototype.moveFocusUp = function() {
		...
		for ( var i = options.index( this.active[0] ) - 1; i >= 0; i -- ) {
			...
			if ( option.attr( 'aria-disabled' ) === 'false' ) {
				...
				this.active = option;
				this.active.attr( 'aria-selected', true ).focus();
				break;
			}
		}
	};
