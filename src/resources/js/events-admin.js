...
jQuery( function ( $ ) {
	...
	$( '[data-wp-lists="list:tribe_events_cat"]' ).each( function () {
		const $list = $( this );
		const $first = $list.find( ':checkbox:checked' ).first();

		if ( ! $first.length ) {
			return;
		}

		const top_position = $list.find( ':checkbox' ).position().top;
		const checked_position = $first.position().top;
	...
	const setup_linked_post_fields = function ( post_type ) {
		const saved_template = $( document.getElementById( 'tmpl-tribe-select-' + post_type ) ).length
			? wp.template( 'tribe-select-' + post_type )
			: null;
		const create_template = $( document.getElementById( 'tmpl-tribe-create-' + post_type ) ).length
			? wp.template( 'tribe-create-' + post_type )
			: null;
		const section = $( document.getElementById( 'event_' + post_type ) );
		const rows = section.find( '.saved-linked-post' );
		...
		section.on( 'click', '.tribe-add-post', function ( e ) {
			...
			if ( dropdown.find( '.nosaved' ).length ) {
				const label = dropdown.find( 'label' );
		...
		function add_sticky_linked_post_data( post_type, container, fields ) {
			...
			const $fields = $( fields );
			const $rows = $fields.filter( 'tr.linked-post.' + container );
			...
			if ( 'object' === typeof sticky_data ) {
				for ( const key in sticky_data ) {
					// Check to see if we have a field of this name
					const $field = $fields.find( 'input[name="' + container + '[' + key + '][]"]' );
		...
		rows.each( function () {
			...
			const dropdown = row.find( '.linked-post-dropdown' );
			if ( dropdown.length ) {
	...
	var toggle_linked_post_fields = function ( event ) {
		...
		if ( ! existingPost && value !== '-1' && value ) {
			...
			// Display the Fields
			$group.find( '.linked-post' ).not( '[data-hidden]' ).show().find( '.tribe-dropdown' );
	...
	function showOrHideAddPostButton( $wrapper ) {
		...
		if ( ! existingPost && ( ! value || value === '-1' ) && currentGroupPosition === linkedPostCount ) {
	...
	if ( typeof tribe_l10n_datatables.datepicker !== 'undefined' ) {
		...
		const $end_date = $( document.getElementById( 'EventEndDate' ) );
		const $event_details = $( document.getElementById( 'tribe_events_event_details' ) );
		...
		const tribeDaysPerMonth = [ 29, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 ];

		// start and end date select sections
		const tribeStartDays = [
		...
		for ( const i in tribe_events_linked_posts.post_types ) {
	...
	window.wp.hooks.addAction(
		'tec.events.admin.linked_posts.add_post',
		'tec',
		( post_type, template_section, new_section ) => {
	...
	$( document )
		.on( 'change', '[id="EventCountry"]', function () {
			const $country = $( this );
			const $container = $country.parents( 'tbody' ).eq( 0 );
			const $state_select = $container.find( '#StateProvinceSelect' );
			const $state_dropdown = $state_select.next( '.select2-container' );
	...
	overwriteCoordinates.$toggle = overwriteCoordinates.$container
	...
	const eventSubmitButton = $( '.wp-admin.events-cal #post #publishing-action input[type="submit"]' );

	eventSubmitButton.on( 'click', function () {
	...
	const $tribe_views = $( document.getElementById( 'tribe-field-tribeEnableViews' ) );

	if ( $tribe_views.length ) {
		const $default_view_select = $( 'select[name="viewOption"]' );
		const $default_mobile_view_select = $( 'select[name="mobile_default_view"]' );
		const $view_inputs = $tribe_views.find( 'input:checkbox' );
		const $view_desc = $( '#tribe-field-tribeEnableViews .tribe-field-wrap p.description' );
		const view_options = {};

		function create_view_array() {
			$default_view_select.find( 'option' ).each( function () {
				const $this = $( this );
				view_options[ $this.attr( 'value' ) ] = $this.text();
			} );

			$default_mobile_view_select.find( 'option' ).each( function () {
				const $this = $( this );
				view_options[ $this.attr( 'value' ) ] = $this.text();
			} );
		}
		...
		function set_selected_views( $this ) {
			...
			const prev_default_view = $default_view_select.find( 'option:selected' ).first().val();

			const prev_default_mobile_view = $default_mobile_view_select.find( 'option:selected' ).first().val();

			$default_view_select.find( 'option' ).remove();
			...
			const $prev_default_option = $default_view_select.find( "option[value='" + prev_default_view + "']" );
			const $prev_default_mobile_option = $default_mobile_view_select.find(
				"option[value='" + prev_default_mobile_view + "']"
			);
			...
			$default_view_select.select2( 'destroy' ).select2( { width: 'auto' } );

			$default_mobile_view_select.select2( 'destroy' ).select2( { width: 'auto' } );
		}

		create_view_array();

		$tribe_views.on( 'change', 'input:checkbox', function () {
			const $this = $( this );
