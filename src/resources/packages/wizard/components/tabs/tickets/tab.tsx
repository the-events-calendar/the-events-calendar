...
const TicketsContent = ( { moveToNextTab, skipToNextTab } ) => {
	const eventTicketsInstalled = useSelect(
		( select ) => select( SETTINGS_STORE_KEY ).getSetting( 'event-tickets-installed' ) || false,
		[]
	);
	const eventTicketsActive = useSelect(
		( select ) => select( SETTINGS_STORE_KEY ).getSetting( 'event-tickets-active' ) || false,
		[]
	);
	...
	const message = ! eventTicketsInstalled
		? __( 'Yes, install and activate Event Tickets for free on my website.', 'the-events-calendar' )
		: __( 'Activate the Event Tickets Plugin for me.', 'the-events-calendar' );
		...
		<>
			...
			<div className="tec-events-onboarding__tab-header">
				...
				<p className="tec-events-onboarding__tab-subheader">
					{ __(
						'Will you be selling tickets or providing attendees the ability to RSVP to your events?',
						'the-events-calendar'
					) }
				</p>
			</div>
				...
				{ ! eventTicketsActive && (
					...
					>
						...
						<div className="tec-events-onboarding__checkbox-description">
							<label htmlFor="tec-events-onboarding__tickets-checkbox-input">{ message }</label>
							<div id="tec-events-onboarding__checkbox-description"></div>
						</div>
					</div>
				) }
