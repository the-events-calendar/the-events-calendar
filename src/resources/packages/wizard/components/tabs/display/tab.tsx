...
const DisplayContent: React.FC = ( { moveToNextTab, skipToNextTab } ) => {
	const availableViews = useSelect(
		( select ) => select( SETTINGS_STORE_KEY ).getSetting( 'availableViews' ) || [],
		[]
	);
	const tribeEnableViews = useSelect(
		( select ) => select( SETTINGS_STORE_KEY ).getSetting( 'tribeEnableViews' ) || [],
		[]
	);
		...
		<>
			<div className="tec-events-onboarding__tab-header">
				...
				<p className="tec-events-onboarding__tab-subheader">
					{ __(
						'Select how you want to display your events on your site. You can choose more than one.',
						'the-events-calendar'
					) }
				</p>
			</div>
				...
				{ ! hasProViews && (
					<div className="tec-events-onboarding__view_upsell">
						<p className="tec-events-onboarding__view_upsell_callout">
							{ __( 'More views available with', 'the-events-calendar' ) }{ ' ' }
							<BoltIcon className="tec-events-onboarding_pro-icon" />
						...
						<div className="tec-events-onboarding__view_upsell_list">
							<div className="tec-events-onboarding__view_upsell-cell">
								{ icons.get( 'map' ) }
								<span className="tec-events-onboarding__view_upsell-label">
									{ proViews.get( 'map' ) }
								</span>
							</div>
							<div className="tec-events-onboarding__view_upsell-cell">
