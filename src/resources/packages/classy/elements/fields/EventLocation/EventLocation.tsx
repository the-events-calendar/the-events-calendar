...
function buildOptionFromFetchedVenue(venue: FetchedVenue): SelectOption {
	return {
...
export function EventLocation(props: { title: string }) {
	...
	const venueIds = useSelect((select) => {
		// @ts-ignore The method comes from a Classy store selectors.
		return select("tec/classy").getEditedPostVenueIds();
	}, []);
	...
	useEffect(() => {
		apiFetch({
			...
			.then((results) => {
				...
				if (
					!(results.hasOwnProperty("venues") && results.hasOwnProperty("total"))
				) {
					console.error(
				...
				if (safeResults.total > options.length + safeResults.venues.length) {
					setPageToFetch(pageToFetch + 1);
				}

				const newVenues = safeResults.venues.map(buildOptionFromFetchedVenue);

				// Udpate the fetched set of venues.
				fetched.current = [...fetched.current, ...safeResults.venues];
	...
	const onVenueSelect = useCallback(
		(newValue: string) => {
			const venueIds = [...currentVenueIds, parseInt(newValue)];
