...
export function EventOrganizer(props: { title: string }) {
	...
	useEffect(() => {
		apiFetch({
			...
			.then((results) => {
				...
				fetched.current = [...fetched.current, ...safeResults.organizers];
	...
	const onOrganizerSelect = useCallback(
		(newValue: string) => {
			...
			const organizerIds = [...currentOrganizerIds, parseInt(newValue)];
							...
							<div className="classy-field__input" ref={ref}>
								...
								>
									...
									<Button
										...
										onClick={() => {
											console.log("Organizer creation not implemented yet.");
										}}
									>
										{_x(
