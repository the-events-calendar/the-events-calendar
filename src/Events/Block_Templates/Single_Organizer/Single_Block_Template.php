...
namespace TEC\Events\Block_Templates\Single_Organizer;

use TEC\Events\Blocks\Single_Organizer\Block;
 ...
 * @since   TBD
 *
 * @package TEC\Events\Block_Templates\Single_Organizer
 */
class Single_Block_Template implements Block_Template_Contract {
	...
	/**
	 * Constructor for Single Organizer Block Template.
	 *
	 * @since TBD
	 *
	 * @param Block $block The registered Block for Single Organizer.
	 */
	...
	/**
	 * Creates then returns the WP_Block_Template object for single organizer.
	 *
	 * @since TBD
	 *
	...
	protected function create_wp_block_template(): ?WP_Block_Template {
		...
		$post_title = sprintf(
			esc_html_x( 'Single %s', 'The Full Site editor organizer block navigation title', 'the-events-calendar' ),
			tribe_get_organizer_label_singular()
		);
		/* translators: %s: event (singular) */
		$post_excerpt = sprintf(
			esc_html_x( 'Displays a single %s.', 'The Full Site editor organizer block navigation description', 'the-events-calendar' ),
		...
		$insert       = [
			...
			'post_content' => Template_Utils::inject_theme_attribute_in_content(
				file_get_contents(
					Tribe__Events__Main::instance()->plugin_path . '/src/Events/Block_Templates/Single_Organizer/templates/single-organizer.html'
				)
			),
			'tax_input'    => [
	...
	public function get_block_template(): ?WP_Block_Template {
		...
		if ( ! $wp_block_template instanceof WP_Block_Template ) {
			do_action(
				'tribe_log',
				'error',
				'Failed locating our WP_Block_Template for the Single Organizer Block',
				[
					'method'    => __METHOD__,
