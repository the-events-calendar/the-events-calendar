...
class Provisional_Post_Cache {
	...
	public function hydrate_caches( array $occurrences_ids = [] ): void {
		...
		[
			...
			$occurrences_ids,
			function ( array $carry, int $occurrence_id ) use ( $cache ): array {
	 ...
	 * @since 6.0.0
	 *
	 * @param Occurrence $occurrence The Occurrence to prime the caches for.
	 */
	private function set_occurrence_cache( Occurrence $occurrence ): void {
		 ...
		 */
		$post = apply_filters( 'tec_events_pro_custom_tables_v1_occurrence_cache_post', $post, $occurrence, $provisional_id );
	 ...
	 * @since 6.0.0
	 *
	 * @param int $post_id The real Event post ID to flush from the cached IDs list.
	 */
