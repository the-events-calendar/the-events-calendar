...
class Organizers extends Post_Entity_Endpoint implements Readable_Endpoint, Creatable_Endpoint {
	...
	public function read_schema(): OpenAPI_Schema {
		...
		$headers_collection[] = new Positive_Integer(
			'X-WP-Total',
			fn() => __( 'The total number of organizers matching the request.', 'the-events-calendar' ),
			null,
			null,
			null,
			true
		...
		$schema->add_response(
			200,
			fn() => __( 'Returns the list of organizers', 'the-events-calendar' ),
			$headers_collection,
			'application/json',
			$response,
		);
	...
	public function create_schema(): OpenAPI_Schema {
		...
		$schema->add_response(
			500,
			fn() => __( 'Failed to create the organizer', 'the-events-calendar' ),
		);
