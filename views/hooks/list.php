...
if( !class_exists('Tribe_Events_List_Template')){
	class Tribe_Events_List_Template extends Tribe_Template_Factory {
		...
		public static function init(){
			...
			elseif ( empty($search_term) && empty( $wp_query->query_vars['s'] ) && !have_posts() ) { // Messages if currently no events, and no search term
			...
			if ( have_posts() ) {
				...
				if( tribe_is_past() ) {
					...
					if( get_previous_posts_link() ) {
						...
						$html .= '<a href="'. tribe_get_upcoming_link() .'" rel="prev">&laquo; '. __( 'Previous Events', 'tribe-events-calendar' ) .'</a>';
				...
				if( tribe_is_past() ) {
					if( get_query_var( 'paged' ) > 1 ) {
						...
						$html .= '<a href="'. tribe_get_past_link() .'" rel="prev">'. __( 'Next Events &raquo;', 'tribe-events-calendar' ) .'</a>';
					} elseif( !get_previous_posts_link() ) {
...
<?php
		...
		public static function before_the_content( $content, $post_id ){
			$html = '<div class="tribe-list-event-description tribe-content entry-summary">';
